...
这是 SDS 2.0 的中文使用说明。SDS（Simple Dynamic Strings）是一个面向 C 的
动态字符串库：对外仍使用 `char *`，在返回指针之前保存一个动态大小的私有
头部。这样既能像普通 C 字符串一样传给 `printf()`，又能通过头部快速获取
长度和剩余容量，并安全保存中间包含空字符的二进制数据。

本中文化版本只翻译说明、源码注释、测试描述和构建提示，不改变公开 C API、
SDS 内存布局、分配策略或测试数据。`LICENSE` 中的 BSD 三条款法律文本保留
原文；文档下方保留上游英文参考，便于逐段对照。

## 快速开始

将下列三个文件加入 C99 项目即可使用：

```text
sds.c       实现
sds.h       公开 API 和内联长度/容量操作
sdsalloc.h  分配器宏配置
```
...
注意：每个可能扩容的函数都返回新的 SDS 指针，调用方必须接收返回值，例
如 `s = sdscat(s, "more")`。内存不足时这些函数可能返回 `NULL`，生产代码
应检查返回值。`sdsfree(NULL)` 是安全的。

## 核心 API

| API | 作用 |
|---|---|
| `sdsnewlen(init, len)` | 按字节长度创建字符串；支持二进制数据 |
| `sdsnew(init)` | 从空字符结尾的 C 字符串创建 |
| `sdsempty()` / `sdsdup(s)` | 创建空字符串 / 复制字符串 |
| `sdslen(s)` / `sdsavail(s)` / `sdsalloc(s)` | 读取已用长度、剩余容量和总容量 |
| `sdsfree(s)` / `sdsclear(s)` | 释放字符串 / 清空内容但保留容量 |
| `sdscatlen()` / `sdscat()` / `sdscatsds()` | 追加指定字节、C 字符串或 SDS |
| `sdscpylen()` / `sdscpy()` | 覆盖内容，尽量复用已有容量 |
| `sdsgrowzero(s, len)` | 扩展到指定长度，新增字节清零 |
| `sdstrim(s, cset)` / `sdsrange(s, start, end)` | 两端裁剪 / 按包含端点的区间截取 |
| `sdstolower()` / `sdstoupper()` | 对每个字节执行大小写转换 |
| `sdscatprintf()` / `sdscatfmt()` | 格式化追加；后者只支持文档列出的格式 |
| `sdsfromlonglong()` | 高效地从整数创建 SDS |
| `sdscatrepr()` / `sdssplitargs()` | 生成带转义表示 / 解析带引号参数 |
| `sdssplitlen()` / `sdsfreesplitres()` | 按二进制安全分隔符拆分并释放结果 |
| `sdsjoin()` / `sdsjoinsds()` | 连接 C 字符串数组或 SDS 数组 |
| `sdsmapchars()` | 按字符映射表原地替换 |

### 二进制安全和空字符

SDS 的逻辑长度由头部保存，而不是通过 `strlen()` 推断，因此可以创建：

```c
sds binary = sdsnewlen("A\0B", 3);
```

此时 `sdslen(binary)` 为 3，`binary[3]` 始终是隐含的结尾空字符；但使用
`printf("%s", binary)` 时，标准 C 字符串规则仍会在中间的空字符处停止。
需要完整内容时应使用 `sdslen()` 配合 `fwrite()` 或其他带长度的 API。

### 区间和分配规则

`sdsrange()` 的 `start`、`end` 均为包含端点的字节下标；负数从末尾计算，
其中 `-1` 表示最后一个字节。SDS 扩容时通常按所需容量的两倍预分配，额外
预分配受 `SDS_MAX_PREALLOC` 限制，默认上限为 1 MiB。需要释放末尾空闲空间
时使用 `sdsRemoveFreeSpace()`，并接收其返回的新指针。

### 格式化追加

`sdscatprintf()` 接受标准 `printf` 风格格式；`sdscatfmt()` 更快，但只接受
以下格式：`%s` C 字符串、`%S` SDS、`%i` 有符号 int、`%I` 64 位有符号数、
`%u` 无符号 int、`%U` 64 位无符号数，以及 `%%`。不要把未列出的格式传给
`sdscatfmt()`。

## 内存和分配器

SDS 返回的指针指向字符串缓冲区，头部位于该指针之前。头部类型按字符串长度
动态选择：类型 5、8、16、32、64 分别使用更小或更大的长度字段。类型 5
不记录剩余容量，因此追加操作会升级到类型 8 或更大类型。

默认情况下，`sdsalloc.h` 将 `s_malloc`、`s_realloc`、`s_free` 映射到 libc
的 `malloc`、`realloc`、`free`。若项目已有内存分配器，只需在该文件中修改
三个宏；`sds_malloc()`、`sds_realloc()`、`sds_free()` 可供外部代码调用与
SDS 一致的分配器。

SDS 的低级 API 使用驼峰命名：`sdsMakeRoomFor()`、`sdsIncrLen()`、
`sdsRemoveFreeSpace()`、`sdsAllocSize()` 和 `sdsAllocPtr()`。这些接口允许
直接写入预留空间或访问分配块，使用时必须自行保证长度、容量和指针生命周期
正确。
...
测试程序为 `sds-test`，会覆盖创建、长度、追加、复制、格式化、裁剪、区间、
比较、转义和预分配等行为。

`sds_test` 在 Windows 启动时会设置 UTF-8 控制台输出代码页，因此测试状态文字
可以正常显示中文；SDS 库本身不会修改宿主程序的控制台状态。若将输出重定向到
文件，查看器仍需按 UTF-8 打开。

### CMake

SDS 已提供独立 CMake 入口，也已接入仓库根 CMake：

```powershell
cmake -S sds-cn -B sds-cn/build/windows-x64-release -DSDS_BUILD_TESTS=ON
cmake --build sds-cn/build/windows-x64-release
ctest --test-dir sds-cn/build/windows-x64-release --output-on-failure
```

根工程中可通过 `-DKILO_BUILD_SDS=OFF` 关闭 SDS 子项目。根工程默认构建
`sds` 静态库和 `sds_test` 测试程序；SDS 的源码、测试和上层 Kilo 不共享
运行时状态。

## 平台和编码边界

SDS 按字节处理字符串，不负责 UTF-8、GBK 或其他文本编码转换；中文文本可以
作为 UTF-8 字节序列保存，但长度和区间下标仍然是字节数，不是 Unicode 码点
或用户感知字符数。Windows/MSVC 构建会以 UTF-8 读取源码，头文件对齐和
`ssize_t` 做了兼容处理；分配器和公开 API 保持不变。

## 版本和许可证

当前代码为 SDS 2.0 独立版本。2.0 使用动态大小头部，内存占用更低，并增加
`sdscatfmt()`；它与 SDS 1.0 不保持二进制兼容，但 API 大体兼容。版权所有者、
贡献者和 Redis 名称的使用限制以 `LICENSE` 为准。
