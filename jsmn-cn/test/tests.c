 ...
 * jsmn 回归测试集合。
 *
 * 测试同时覆盖默认模式、JSMN_STRICT 模式、JSMN_PARENT_LINKS 模式及两者
 * 组合。每个测试通过标记类型、边界、子项数量和原始文本验证解析结果。
 */
...
/* 逐步增加输入长度，验证不完整字符串返回 JSMN_ERROR_PART。 */
...
/* 默认模式允许未加引号的键和值，以兼容 JavaScript 风格输入。 */
...
/* 指定输入长度时，只解析范围内的 JSON，不要求 NUL 结尾。 */
...
int main(void) {
  test(test_empty, "空对象和空数组");
  test(test_object, "对象和键值对");
  test(test_array, "数组");
  test(test_primitive, "JSON 原始值");
  test(test_string, "JSON 字符串");

  test(test_partial_string, "不完整 JSON 字符串");
  test(test_partial_array, "不完整数组");
  test(test_array_nomem, "标记不足时的数组解析");
  test(test_unquoted_keys, "未加引号的键（JavaScript 风格）");
  test(test_input_length, "非 NUL 结尾的输入范围");
  test(test_issue_22, "历史问题 #22");
  test(test_issue_27, "历史问题 #27");
  test(test_count, "标记数量统计");
  test(test_nonstrict, "宽松模式");
  test(test_unmatched_brackets, "未匹配括号");
  test(test_object_key, "对象键类型");
